...
return new class extends Migration
{
    ...
    {
       // if (App::isProduction()) {
            // Check if event already exists with same title, date and start time
            $existingEvent = Event::where('title', 'Skånelägret 200')
                ->where('startdate', '2025-05-16')
                ->whereHas('routeDetail', function ($query) {
                    $query->where('start_time', '08:00');
                })
                ->first();

            if ($existingEvent) {
                Log::info('Migration skipped: Event "Skånelägret 200" on 2025-05-16 at 08:00 already exists.');
                return;
            }
            ...
            $event->title = 'Skånelägret 200';
            $event->description = '';
            $event->startdate = '2025-05-16';
            $event->enddate = '2025-05-16';
            $event->completed = false;
            $event->embedid = '';
            $event->event_type = 'BRM'; // BRM = Brevet Randonneurs Mondiaux

            // Step 2: Hardcode county ID (Skåne)
            $event->county_id = 10; // Hardcoded county ID - change this later

            // Step 3: Hardcode organizer ID (Randonneur Väst)
            $event->organizer_id = 5; // Hardcoded organizer ID - change this later
           ...
           // $routeDetail->height_difference = ; // Hardcoded value in meters
            $routeDetail->start_time = '08:00';
            $routeDetail->start_place = 'Hotell Erikslund';
            $routeDetail->pay_link = 'https://www.randonneurgoth.com/#h.rkx1uy9hf0wn';
