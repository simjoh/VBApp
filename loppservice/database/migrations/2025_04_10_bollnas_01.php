...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (App::isProduction()) {
            ...
            $event = new Event();
            $event->event_uid = '04c75225-b563-43b6-aaa8-cbada83dacfd'; // Make sure this is unique
            $event->title = 'BRM 200 Bollnäs Grus';
            $event->description = '';
            ...
            // Step 2: Hardcode county ID (Gävleborgs län)
            $event->county_id = 17; // Hardcoded county ID - change this later

            // Step 3: Hardcode organizer ID (Bollnäs CK)
            $event->organizer_id = 14; // Hardcoded organizer ID - change this later
            ...
            $eventconfiguration->registration_opens = '2025-08-23 00:00:00';
            $eventconfiguration->registration_closes = '2025-08-22 15:00:00';
            ...
            $routeDetail->height_difference = 2607; // Hardcoded value in meters
            $routeDetail->start_time = '08:00';
            $routeDetail->start_place = 'Scandic Hotell, Bollnäs';
            $routeDetail->pay_link = 'Swish 123 036 90 09';
	        $routeDetail->name = $event->title;

            // Simplified description without start time/place info
            $routeDetail->description = 'OBS ca 65% grusväg, många höjdmeter och mycket ödemark.';

            // Hardcoded track link
            $routeDetail->track_link = 'https://ridewithgps.com/routes/37109987';
            $routeDetail->save();
