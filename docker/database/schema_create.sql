...
CREATE
ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_dns_on_event_and_track`  AS
...
CREATE
ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_partisipant_passed_checkpoints`  AS
...
CREATE
ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_partisipant_to_pass_checkpoint`  AS
SELECT `tr`.`track_uid`            AS `track_uid`,
       `pach`.`participant_uid`    AS `participant_uid`,
       `sit`.`site_uid`            AS `site_uid`,
       `cpo`.`checkpoint_uid`      AS `checkpoint_uid`,
...
CREATE
ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_race_statistic`  AS
...
CREATE
ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_result_for_event_and_track`  AS
...
CREATE
ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_track_contestant_on_event_and_track`  AS
SELECT `sa`.`startnumber`        AS `startnumber`,
