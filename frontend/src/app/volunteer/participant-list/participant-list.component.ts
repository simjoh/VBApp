...
export class ParticipantListComponent implements OnInit {
	...
	chekedoutSubject = new BehaviorSubject(false);
	checkeout$ = this.chekedoutSubject.asObservable().pipe(
		map((val) => {
			let text = "";
			if (val === false) {
				text = 'Checout'
	...
	randonneurs = combineLatest([this.volonteerComponentService.valdkontroll$, this.volonteerComponentService.randonneurs$, this.volonteerComponentService.stats$]).pipe(
		map(([all, insert, mesure]) => {
			...
			this.ss.choosenControl = all.site.adress;
	...
	async checkout(product: any, event: Event) {
